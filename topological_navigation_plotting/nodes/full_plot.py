...
class Plotter:
    def __init__(self):
        ...
        rospy.Subscriber("/currently_detected_galleries",
                         Float32MultiArray,
                         callback=self.detected_galleries_callback)
        rospy.Subscriber("/tracked_galleries",
                         Float32MultiArray,
                         callback=self.tracked_galleries_callback)
        rospy.Subscriber("/back_gallery",
                         Float32,
                         callback=self.back_gallery_callback)
        rospy.Subscriber("/followed_gallery",
                         Float32,
    ...
    def back_gallery_callback(self, msg):
        self._back_gallery = np.array([msg.data, 0.8])
        self._updated_back_gallery = True

    def followed_gallery_callback(self, msg):
        self._followed_gallery = np.array([msg.data, 0.8])
    ...
    def draw_loop(self):
        ...
        self._nn_output_lines, = self._ax1.plot([], lw=6)
        self._currently_detected_scatter = self._ax1.scatter(
            [], [], c="b", s=300)
        self._tracked_galleries_scatter = self._ax1.scatter(
            [], [], c="r", s=300)
        ...
        while not rospy.is_shutdown():
            ...
            if self._updated_detected_galleries:
                self._currently_detected_scatter.set_offsets(
                    self._detected_galleries)
            if self._updated_tracked_galleries:
                self._tracked_galleries_scatter.set_offsets(
                    self._tracked_galleries
                )
            if self._updated_back_gallery:
                self._back_gallery_scatter.set_offsets(self._back_gallery)
            if self._updated_followed_gallery:
                self._followed_gallery_scatter.set_offsets(
                    self._followed_gallery)
            # restore background
