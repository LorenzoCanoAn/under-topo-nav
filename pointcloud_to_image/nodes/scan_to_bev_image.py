...
class ScanToBEVImageNode:
    ...
    def setup_sub_pub(self):
        ...
        self._main_subscriber = rospy.Subscriber(
            self.input_topic, LaserScan, callback=self.callback
        )
        self._reconfig_conversor_sub = rospy.Subscriber(
            "~reconfigure_conversor", Empty, callback=self.setup_conversor, queue_size=1
        )
