...
def gallery_vector_callback(msg: ros_std_msg.Float32MultiArray):
    ...
    vel_msg.angular.z = (
        final_angle / abs(final_angle) * min([abs(final_angle), MAX_ANG_VEL])
    )
    vel_msg.linear.x = (MAX_VEL * (min(min_dist, 2) / 2)) - abs(
        vel_msg.angular.z
    ) / MAX_ANG_VEL * MAX_VEL
...
def main():
    ...
    rospy.Subscriber(
        ...
        ros_std_msg.Float32MultiArray,
        callback=gallery_vector_callback,
    )
    rospy.Subscriber("/scan", ros_sns_msg.LaserScan, callback=laser_scan_callback)
    time.sleep(1)
    plt.show()
    # rospy.spin()
